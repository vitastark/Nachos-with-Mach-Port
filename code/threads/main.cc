...
//----------------------------------------------------------------------
// Server
// function for assignment3
// communicate with other Nachos processes
// Author: Haiyang Sun
// Date: 2018/11/25
//----------------------------------------------------------------------
...
void Server(int num){
    while(true){
        cout<<"now Server is running"<<endl;
        while(kernel->msg_queue.empty() == false){
            
            Message msg = kernel->msg_queue.front();
            kernel->msg_queue.pop();
            int port = msg.getPort();
            string body = msg.getMsgBody();
            if(kernel->mach_ports[port].getMsg() == ""){
                kernel->mach_ports[port].setMsg(body);
            }
            
            Thread* t = kernel->mach_ports[port].getReceiveThread();
            if(t == NULL){
                cout<<"receivethread is not decided yet, Server Yield"<<endl;
            }
            else{
        ...
        cout<<"msg_queue is empty, Server Yield"<<endl;
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
            char *userProgName = argv[i+1];
            name[num] = userProgName;
            num++;
	    i++;
	}
    ...
    t2->Fork((VoidFunctionPtr) Server, (void*) 1);
    // finally, run an initial user program if requested to do so
    for(int i=0;i<num;i++){
       //cout<<name[i]<<endl;
       Thread* t1 = new Thread("forked thread");
       t1->Fork((VoidFunctionPtr) RunUserProg, name[i]);
    }
